...
public class MStoreManageSettingNameActivity extends AppCompatActivity {
    ...
    @BindView(R.id.et_mStoreManageSettingName)
    TextView tv_name;
    ...
    private void settingNameMessage(){
        //  create new thread
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try{
                    ...
                    String setNameMsg = "mechantName=" + tv_name.getText().toString() ;
                    outputStream.write(setNameMsg.getBytes());
                    outputStream.flush();
                    outputStream.close();
                    ...
                    if(result!=null){
                        switch (result){
                            case "200":
                                Toast.makeText(MStoreManageSettingNameActivity.this, "保存成功！", Toast.LENGTH_SHORT).show();
                                break;
                            case "404":
                                //  username not exited
                                Toast.makeText(MStoreManageSettingNameActivity.this, "保存失败，网络开小差啦！", Toast.LENGTH_SHORT).show();
                                break;
                            case "401":
                                //  wrong password
                                Toast.makeText(MStoreManageSettingNameActivity.this, "保存失败，网络开小差啦！", Toast.LENGTH_SHORT).show();
                    ...
                    e.printStackTrace();
                    Log.d("SettingNameActivity", e + e.getMessage());
                }
            }
        }).start();
    }
